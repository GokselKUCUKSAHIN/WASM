...
function toGray(r: u8, g: u8, b: u8): u8 {
    return u8(r * 0.2126 + g * 0.7152 + b * 0.0722);
}

export function gscale(width: i32, height: i32): void {
...
export function imbin(width: i32, height: i32): void {
    for (let i = 0; i < width * height * 4; i += 4 /*rgba*/) {
        const r = load<u8>(i);
        const g = load<u8>(i + 1);
        const b = load<u8>(i + 2);
        const gray = toGray(r, g, b);
        const bin = u8(gray < 128 ? 0 : 255);
        store<u8>(i, bin);
        store<u8>(i + 1, bin);
        store<u8>(i + 2, bin);
        store<u8>(i + 3, 255);
